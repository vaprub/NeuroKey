...
def force_fix_torch_dll():
    """
    Максимально агрессивный фикс для c10.dll
    Выполняется до любых импортов torch и transformers
    """
    try:
...
def guess_platform(key: str) -> str:
    """Определяет вероятную платформу по формату ключа."""
...
def guess_game_name(title: str, description: str, key: str) -> str:
    """
    Простейшая эвристика: пытается извлечь название игры из заголовка или описания.
    """
...
class ModelManager:
    """
    Менеджер моделей машинного обучения для анализа раздач.
    Работает в двух режимах: полный (с torch) и упрощенный (без torch).
    """
    ...
    def __init__(self, model_dir: str = "models", use_gpu: bool = True):
        ...
        # Инициализируем модели (будут загружены при первом использовании)
        self.encoder = None
        self.classifier = None
        self.ner_pipeline = None
        
        # Ключевые слова для упрощенного анализа
        ...
        # Пробуем загрузить модели если всё есть
    ...
    def _load_models(self):
        ...
        try:
            # Загружаем Sentence Transformer для поиска (дообученная модель)
            ...
            if search_model_path.exists():
                ...
                # Если нет дообученной, грузим базовую
                self.encoder = SentenceTransformer(
                    'all-MiniLM-L6-v2',
                    device='cuda' if self.use_gpu and torch.cuda.is_available() else 'cpu'
                )
            ...
            # Загружаем классификатор (опционально, пока оставим базовый)
            ...
            # Загружаем NER для поиска ключей (дообученная модель)
    ...
    def analyze_relevance(self, text: str) -> float:
        # (полностью совпадает с предыдущей версией)
        if not text:
            return 0.0
        ...
        if TORCH_AVAILABLE and self.classifier:
            try:
    ...
    def extract_keys(self, text: str) -> list:
        """Извлекает ключи из текста, используя NER модель, если доступна."""
        keys = []
        
        # Используем NER если доступен
        if TORCH_AVAILABLE and self.ner_pipeline:
            try:
        ...
        # Дополнительно используем регулярные выражения
        key_patterns = [
    ...
    def get_embedding(self, text: str) -> np.ndarray:
        if not TORCH_AVAILABLE or not self.encoder:
            return np.zeros(384)
        try:
